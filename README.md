...
透過 `bootstrap.ps1` 跑一鍵安裝時，`install.ps1` 是被 `bootstrap.ps1` 呼叫的，兩者共用同一份 `wezterm-install-*.log`（`install.ps1` 偵測到 transcript 已經在跑就不會另外開一份，不會產生兩份重複的 log）；直接單獨執行 `install.ps1` 才會產生自己的 log。
